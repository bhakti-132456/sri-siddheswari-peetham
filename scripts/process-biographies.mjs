    ...
    for (let i = 1; i <= swamiCount; i++) {
        ...
        if (fs.existsSync(filePath)) {
            try {
                ...
                allBiographies[lang][`swami${i}`] = formatToHtml(rawText);
            ...
            allBiographies[lang][`swami${i}`] = "";
        }
    }
}

// Generate TypeScript file
let tsContent = `// Auto-generated by scripts/process-biographies.mjs
// Do not edit manually - run 'npm run process-bios' to regenerate

export const biographies: Record<string, Record<string, string>> = {\n`;

for (const lang of languages) {
    tsContent += `  ${lang}: {\n`;
    for (let i = 1; i <= swamiCount; i++) {
        const content = allBiographies[lang][`swami${i}`] || "";
        // Clean up escaped quotes from source text, then escape for template literals
        const cleaned = content.replace(/\\\\"/g, '"');  // Convert \\\" to just "
        const escaped = cleaned.replace(/`/g, '\\`').replace(/\$\{/g, '\\${');  // Only escape template literal syntax
        tsContent += `    swami${i}: \`${escaped}\`,\n`;
    }
    tsContent += `  },\n`;
}
tsContent += `};\n`;

const outputPath = path.join(outputDir, 'biographies.ts');
